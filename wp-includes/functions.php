...
function do_feed()
{
    ...
    if (! has_action("do_feed_{$feed}")) {
        wp_die(__('ERROR: This is not a valid feed template.'), '', [ 'response' => 404 ]);
    }
    ...
    if ($unique_filename_callback && is_callable($unique_filename_callback)) {
        ...
        if ($ext && strtolower($ext) != $ext) {
            ...
            while (file_exists($dir . "/$filename") || file_exists($dir . "/$filename2")) {
                ...
                $filename2 = str_replace([ "-$number$ext2", "$number$ext2" ], "-$new_number$ext2", $filename2);
                $number = $new_number;
            }
        ...
        while (file_exists($dir . "/$filename")) {
            if ('' == "$number$ext") {
                ...
                $filename = str_replace([ "-$number$ext", "$number$ext" ], "-" . ++$number . $ext, $filename);
            }
        }
    }
...
function wp_upload_bits($name, $deprecated, $bits, $time = null)
{
     ...
     */
    $upload_bits_error = apply_filters('wp_upload_bits', [ 'name' => $name, 'bits' => $bits, 'time' => $time ]);
...
function wp_widgets_add_menu()
{
    ...
    $submenu['themes.php'][7] = [ __('Widgets'), 'edit_theme_options', 'widgets.php' ];
...
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
	<title><?php _e('Database Error');
    ?></title>

</head>
<body>
	<h1><?php _e('Error establishing a database connection');
    ?></h1>
</body>
</html>
<?php
...
function wp_auth_check_html()
{
	...
	<button type="button" class="wp-auth-check-close button-link"><span class="screen-reader-text"><?php _e('Close dialog');
    ?></span></button>
	<?php

    if ($same_domain) {
        ?>
		<div id="wp-auth-check-form" class="loading" data-src="<?php echo esc_url(add_query_arg([ 'interim-login' => 1 ], $login_url));
        ?>"></div>
		<?php
	...
	<div class="wp-auth-fallback">
		<p><b class="wp-auth-fallback-expired" tabindex="0"><?php _e('Session expired');
    ?></b></p>
		<p><a href="<?php echo esc_url($login_url);
    ?>" target="_blank"><?php _e('Please log in again.');
    ?></a>
