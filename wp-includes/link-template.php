...
function get_permalink($post = 0, $leavename = false)
{
    ...
    } elseif (in_array($post->post_type, get_post_types(['_builtin' => false]))) {
    ...
    if ('' != $permalink && !in_array($post->post_status, [ 'draft', 'pending', 'auto-draft', 'future' ])) {
...
function _get_page_link($post = false, $leavename = false, $sample = false)
{
    ...
    $draft_or_pending = in_array($post->post_status, [ 'draft', 'pending', 'auto-draft' ]);
...
function get_post_comments_feed_link($post_id = 0, $feed = '')
{
    ...
    if ('' != get_option('permalink_structure')) {
        ...
        if ($unattached) {
            $url = add_query_arg([ 'feed' => $feed, 'attachment_id' => $post_id ], home_url('/'));
        } elseif ('page' == $post->post_type) {
            $url = add_query_arg([ 'feed' => $feed, 'page_id' => $post_id ], home_url('/'));
        } else {
            $url = add_query_arg([ 'feed' => $feed, 'p' => $post_id ], home_url('/'));
        }
    }
...
function get_adjacent_post_rel_link($title = '%title', $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category')
{
    ...
    $post_title = the_title_attribute([ 'echo' => false, 'post' => $post ]);
