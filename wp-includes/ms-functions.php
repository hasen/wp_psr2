...
function wpmu_validate_blog_signup($blogname, $blog_title, $user = '')
{
    ...
    if (! empty($signup)) {
        ...
        // If registered more than two days ago, cancel registration and let this signup go through.
        if ($diff > 2 * DAY_IN_SECONDS) {
            $wpdb->delete($wpdb->signups, [ 'domain' => $mydomain, 'path' => $path ]);
        } else {
...
function wpmu_activate_signup($key)
{
    ...
    if (empty($signup->domain)) {
        $wpdb->update($wpdb->signups, ['active' => 1, 'activated' => $now], ['activation_key' => $key]);
...
function wpmu_create_blog($domain, $path, $title, $user_id, $meta = [], $site_id = 1)
{
    ...
    foreach ($meta as $key => $value) {
        if (in_array($key, [ 'public', 'archived', 'mature', 'spam', 'deleted', 'lang_id' ])) {
...
function insert_blog($domain, $path, $site_id)
{
    ...
    $result = $wpdb->insert($wpdb->blogs, ['site_id' => $site_id, 'domain' => $domain, 'path' => $path, 'registered' => current_time('mysql')]);
    if (! $result) {
        return false;
    }
...
function wpmu_log_new_registrations($blog_id, $user_id)
{
    ...
    if ($user) {
        $wpdb->insert($wpdb->registration_log, ['email' => $user->user_email, 'IP' => preg_replace('/[^0-9.,]/', '', wp_unslash($_SERVER['REMOTE_ADDR'])), 'blog_id' => $blog_id, 'date_registered' => current_time('mysql')]);
    }
}
...
function global_terms($term_id, $deprecated = '')
{
    ...
    if ($global_id == null) {
        ...
        if (null == $used_global_id) {
            $wpdb->insert($wpdb->sitecategories, [ 'cat_ID' => $term_id, 'cat_name' => $c->name, 'category_nicename' => $c->slug ]);
            ...
            $wpdb->insert($wpdb->sitecategories, [ 'cat_ID' => $new_global_id, 'cat_name' => $c->name, 'category_nicename' => $c->slug ]);
            $global_id = $wpdb->insert_id;
        }
    ...
    if ($global_id != $term_id) {
        ...
        $wpdb->update($wpdb->terms, ['term_id' => $global_id], ['term_id' => $term_id]);
        $wpdb->update($wpdb->term_taxonomy, ['term_id' => $global_id], ['term_id' => $term_id]);
        $wpdb->update($wpdb->term_taxonomy, ['parent' => $global_id], ['parent' => $term_id]);
...
function maybe_add_existing_user_to_blog()
{
    ...
    wp_die(sprintf(__('You have been added to this site. Please visit the <a href="%s">homepage</a> or <a href="%s">log in</a> using your username and password.'), home_url(), admin_url()), __('WordPress &rsaquo; Success'), [ 'response' => 200 ]);
}
