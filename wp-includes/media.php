...
{
    global $_wp_additional_image_sizes;
    $image_sizes = ['thumbnail', 'medium', 'medium_large', 'large']; // Standard sizes
...
function wp_get_audio_extensions()
{
     ...
     *                          'mp3', 'ogg', 'wma', 'm4a', 'wav'.
     */
    return apply_filters('wp_audio_extensions', [ 'mp3', 'ogg', 'wma', 'm4a', 'wav' ]);
}
...
function wp_get_video_extensions()
{
     ...
     */
    return apply_filters('wp_video_extensions', [ 'mp4', 'm4v', 'webm', 'ogv', 'wmv', 'flv' ]);
}
...
function wp_video_shortcode($attr, $content = '')
{
    ...
    // These ones should just be omitted altogether if they are blank
    foreach ([ 'poster', 'loop', 'autoplay', 'preload' ] as $a) {
        if (empty($html_atts[$a])) {
            unset($html_atts[$a]);
        }
    }
...
function wp_prepare_attachment_for_js($attachment)
{
    ...
    if ($meta && 'image' === $type) {
        ...
        $response = array_merge($response, [ 'sizes' => $sizes ], $sizes['full']);
