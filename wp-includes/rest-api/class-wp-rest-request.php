...
class WP_REST_Request implements ArrayAccess
{
    ...
    protected function get_parameter_order()
    {
        ...
        $accepts_body_data = [ 'POST', 'PUT', 'PATCH' ];
    ...
    public function has_valid_params()
    {
        ...
        if (! empty($required)) {
            return new WP_Error('rest_missing_callback_param', sprintf(__('Missing parameter(s): %s'), implode(', ', $required)), [ 'status' => 400, 'params' => $required ]);
        }
        ...
        if ($invalid_params) {
            return new WP_Error('rest_invalid_param', sprintf(__('Invalid parameter(s): %s'), implode(', ', array_keys($invalid_params))), [ 'status' => 400, 'params' => $invalid_params ]);
        }
