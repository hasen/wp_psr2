...
class WP_REST_Server
{
    ...
    public function dispatch($request)
    {
        ...
        foreach ($this->get_routes() as $route => $handlers) {
            ...
            foreach ($handlers as $handler) {
                ...
                if (! is_callable($callback)) {
                    $response = new WP_Error('rest_invalid_handler', __('The handler for the route is invalid'), [ 'status' => 500 ]);
                }
                ...
                if (! is_wp_error($response)) {
                    ...
                    if (! empty($handler['permission_callback'])) {
                        ...
                        if (is_wp_error($permission)) {
                            ...
                            $response = new WP_Error('rest_forbidden', __('Sorry, you are not allowed to do that.'), [ 'status' => 403 ]);
                        }
                    }
                }
        ...
        return $this->error_to_response(new WP_Error('rest_no_route', __('No route was found matching the URL and request method'), [ 'status' => 404 ]));
    }
