...
function get_comment_author_url($comment_ID = 0)
{
    ...
    if (! empty($comment)) {
        ...
        $url = esc_url($author_url, [ 'http', 'https' ]);
        $id = $comment->ID;
    }
...
function comments_template($file = '/comments.php', $separate_comments = false)
{
    ...
    if ($user_ID) {
        $comment_args['include_unapproved'] = [ $user_ID ];
    } elseif (! empty($comment_author_email)) {
        $comment_args['include_unapproved'] = [ $comment_author_email ];
    }
...
function comment_form($args = [], $post_id = null)
{
    ...
    ?>" class="<?php echo esc_attr($args['class_form']);
    ?>"<?php echo $html5 ? ' novalidate' : '';
    ?>>
					<?php
                    /**
