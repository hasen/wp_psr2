...
function redirect_canonical($requested_url = null, $do_redirect = true)
{
    ...
    if (isset($_SERVER['REQUEST_METHOD']) && ! in_array(strtoupper($_SERVER['REQUEST_METHOD']), [ 'GET', 'HEAD' ])) {
        return;
    }
    ...
    if (is_feed() && ($id = get_query_var('p'))) {
        if ($redirect_url = get_post_comments_feed_link($id, get_query_var('feed'))) {
            $redirect['query'] = _remove_qs_args_if_not_in_url($redirect['query'], [ 'p', 'page_id', 'attachment_id', 'pagename', 'name', 'post_type', 'feed'], $redirect_url);
    ...
    if (is_singular() && 1 > $wp_query->post_count && ($id = get_query_var('p'))) {
        ...
        if (isset($vars[0]) && $vars = $vars[0]) {
            ...
            if ($redirect_url = get_permalink($id)) {
                $redirect['query'] = _remove_qs_args_if_not_in_url($redirect['query'], [ 'p', 'page_id', 'attachment_id', 'pagename', 'name', 'post_type' ], $redirect_url);
            }
        }
    }
    ...
    if (is_404()) {
        ...
        if (get_query_var('day') && get_query_var('monthnum') && get_query_var('year')) {
            ...
            if (! wp_checkdate($month, $day, $year, $date)) {
                ...
                $redirect['query'] = _remove_qs_args_if_not_in_url($redirect['query'], [ 'year', 'monthnum', 'day' ], $redirect_url);
            }
        } elseif (get_query_var('monthnum') && get_query_var('year') && 12 < get_query_var('monthnum')) {
            $redirect_url = get_year_link(get_query_var('year'));
            $redirect['query'] = _remove_qs_args_if_not_in_url($redirect['query'], [ 'year', 'monthnum' ], $redirect_url);
        }

        if (! $redirect_url) {
            if ($redirect_url = redirect_guess_404_permalink()) {
                $redirect['query'] = _remove_qs_args_if_not_in_url($redirect['query'], [ 'page', 'feed', 'p', 'page_id', 'attachment_id', 'pagename', 'name', 'post_type' ], $redirect_url);
            }
        }
