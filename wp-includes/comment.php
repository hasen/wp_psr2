...
function wp_delete_comment($comment_id, $force_delete = false)
{
    ...
    if (!$force_delete && EMPTY_TRASH_DAYS && !in_array(wp_get_comment_status($comment), [ 'trash', 'spam' ])) {
...
function wp_update_comment($commentarr)
{
    ...
    $keys = [ 'comment_post_ID', 'comment_content', 'comment_author', 'comment_author_email', 'comment_approved', 'comment_karma', 'comment_author_url', 'comment_date', 'comment_date_gmt', 'comment_type', 'comment_parent', 'user_id', 'comment_agent', 'comment_author_IP' ];
...
function discover_pingback_server_uri($url, $deprecated = '')
{
    ...
    $response = wp_safe_remote_head($url, [ 'timeout' => 2, 'httpversion' => '1.0' ]);
    ...
    $response = wp_safe_remote_get($url, [ 'timeout' => 2, 'httpversion' => '1.0' ]);
...
function do_trackbacks($post_id)
{
    ...
    if (empty($to_ping)) {
        $wpdb->update($wpdb->posts, ['to_ping' => ''], ['ID' => $post_id]);
        return;
    }
...
function pingback($content, $post_ID)
{
     ...
     * @param int   $post_ID     The post ID.
     */
    do_action_ref_array('pre_ping', [ &$post_links, &$pung, $post_ID ]);
