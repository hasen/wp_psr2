...
if (! in_array($tax->name, get_taxonomies([ 'show_ui' => true ]))) {
...
add_screen_option('per_page', [ 'default' => 20, 'option' => 'edit_' . $tax->name . '_per_page' ]);
...
case 'add-tag':
    ...
    if ($ret && !is_wp_error($ret)) {
        ...
        $location = add_query_arg([ 'error' => true, 'message' => 4 ], $location);
    }
...
case 'editedtag':
    ...
    if ($ret && !is_wp_error($ret)) {
        ...
        $location = add_query_arg([ 'error' => true, 'message' => 5 ], $location);
    }
    break;
}
...
<?php $_SERVER['REQUEST_URI'] = remove_query_arg([ 'message', 'error' ], $_SERVER['REQUEST_URI']);
endif; ?>
...
if (current_user_can($tax->cap->edit_terms)) {
    if ('category' == $taxonomy) {
         ...
         */
        do_action('add_category_form_pre', (object) [ 'parent' => 0 ]);
    } elseif ('link_category' == $taxonomy) {
        /**
         ...
         * @param object $arg Optional arguments cast to an object.
         */
        do_action('add_link_category_form_pre', (object) [ 'parent' => 0 ]);
    } else {
        /**
...
do_action("{$taxonomy}_add_form_fields", $taxonomy);
     ...
     * @param object $arg Optional arguments cast to an object.
     */
    do_action('edit_category_form', (object) [ 'parent' => 0 ]);
    } elseif ('link_category' == $taxonomy) {
        /**
     ...
     * @param object $arg Optional arguments cast to an object.
     */
    do_action('edit_link_category_form', (object) [ 'parent' => 0 ]);
    } else {
        /**
...
if (! is_null($tax->labels->popular_items)) {
    if (current_user_can($tax->cap->edit_terms)) {
        $tag_cloud = wp_tag_cloud([ 'taxonomy' => $taxonomy, 'post_type' => $post_type, 'echo' => false, 'link' => 'edit' ]);
    } else {
