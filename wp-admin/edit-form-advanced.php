...
if (post_type_supports($post_type, 'revisions') && 'auto-draft' != $post->post_status) {
    ...
    if (count($revisions) > 1) {
        ...
        $publish_callback_args = [ 'revisions_count' => count($revisions), 'revision_id' => key($revisions) ];
...
<div id="titlewrap">
    ...
    ?>
	<label class="screen-reader-text" id="title-prompt-text" for="title"><?php echo $title_placeholder;
    ?></label>
...
<table id="post-status-info"><tbody><tr>
	<td id="wp-word-count" class="hide-if-no-js"><?php printf(__('Word count: %s'), '<span class="word-count">0</span>');
    ?></td>
