...
wp_reset_vars([ 'action', 'error', 'file', 'plugin' ]);
...
if (empty($plugins)) {
    ...
    ?></h1>
		<div id="message" class="error"><p><?php _e('You do not appear to have any plugins available at this time.');
    ?></p></div>
	</div>
	<?php
...
case 'update':
    ...
    if (is_writeable($real_file)) {
        ...
        if (is_plugin_active($file) || isset($_POST['phperror'])) {
            ...
            if (! is_network_admin()) {
                update_option('recently_activated', [ $file => time() ] + (array) get_option('recently_activated'));
            } else {
                update_site_option('recently_activated', [ $file => time() ] + (array) get_site_option('recently_activated'));
            }
 ...
 <div id="message" class="updated"><p><?php _e('This plugin has been deactivated because your changes resulted in a <strong>fatal error</strong>.') ?></p>
	<?php
        if (wp_verify_nonce($_GET['_error_nonce'], 'plugin-activation-error_' . $file)) {
            ...
            ?>
	<iframe style="border:0" width="100%" height="70px" src="<?php echo esc_url($iframe_url);
            ?>"></iframe>
	<?php 
        } ?>
</div>
...
<?php if (is_writeable($real_file)) : ?>
    ...
    ?>
		<p><?php _e('<strong>Warning:</strong> Making changes to active plugins is not recommended. If your changes cause a fatal error, the plugin will be automatically deactivated.');
    ?></p>
	<?php 
} ?>
	<p class="submit">
