...
function wpmu_delete_blog($blog_id, $drop = false)
{
    ...
    $users = get_users([ 'blog_id' => $blog_id, 'fields' => 'ids' ]);
    ...
    if ($drop) {
        ...
        $wpdb->delete($wpdb->blogs, [ 'blog_id' => $blog_id ]);
...
function send_confirmation_on_profile_email()
{
    ...
    if ($current_user->user_email != $_POST['email']) {
        if (!is_email($_POST['email'])) {
            $errors->add('user_email', __("<strong>ERROR</strong>: The email address isn&#8217;t correct."), [ 'form-field' => 'email' ]);
            return;
        }

        if ($wpdb->get_var($wpdb->prepare("SELECT user_email FROM {$wpdb->users} WHERE user_email=%s", $_POST['email']))) {
            $errors->add('user_email', __("<strong>ERROR</strong>: The email address is already used."), [ 'form-field' => 'email' ]);
...
function upload_space_setting($id)
{
    ...
    ?>
	<tr>
		<th><label for="blog-upload-space-number"><?php _e('Site Upload Space Quota');
    ?></label></th>
		<td>
    ...
    ?>" />
			<span id="blog-upload-space-desc"><span class="screen-reader-text"><?php _e('Size in megabytes');
    ?></span> <?php _e('MB (Leave blank for network default)');
    ?></span>
		</td>
	</tr>
	<?php
...
function choose_primary_blog()
{
	...
	<?php /* translators: My sites label */ ?>
		<th scope="row"><label for="primary_blog"><?php _e('Primary Site');
    ?></label></th>
		<td>
		<?php
    ...
    ?><option value="<?php echo $blog->userblog_id ?>"<?php selected($primary_blog, $blog->userblog_id);
...
function confirm_delete_users($users)
{
	...
	<?php if (1 == count($users)) : ?>
		<p><?php _e('You have chosen to delete the user from all networks and sites.');
    ?></p>
	<?php else : ?>
        ...
        if (! empty($blogs)) {
            ...
            ?></legend></p>
                ...
                foreach ((array) $blogs as $key => $details) {
                    $blog_users = get_users([ 'blog_id' => $details->userblog_id, 'fields' => [ 'ID', 'user_login' ] ]);
                    if (is_array($blog_users) && !empty($blog_users)) {
						...
						<ul style="list-style:none;">
							<li><?php printf(__('Site: %s'), $user_site);
                        ?></li>
            ...
            ?>
				<td><fieldset><p><legend><?php _e('User has no sites or content and will be deleted.');
            ?></legend></p>
			<?php 
        }
        ?>
    ...
    if (1 == count($users)) : ?>
		<p><?php _e('Once you hit &#8220;Confirm Deletion&#8221;, the user will be permanently removed.');
    ?></p>
	<?php else : ?>
...
function network_edit_site_nav($args = [])
{
    ...
    $links = apply_filters('network_edit_site_nav_links', [
        'site-info'     => [ 'label' => __('Info'),     'url' => 'site-info.php',     'cap' => 'manage_sites' ],
        'site-users'    => [ 'label' => __('Users'),    'url' => 'site-users.php',    'cap' => 'manage_sites' ],
        'site-themes'   => [ 'label' => __('Themes'),   'url' => 'site-themes.php',   'cap' => 'manage_sites' ],
        'site-settings' => [ 'label' => __('Settings'), 'url' => 'site-settings.php', 'cap' => 'manage_sites' ],
    ]);
    ...
    foreach ($r['links'] as $link_id => $link) {
        ...
        $url = add_query_arg([ 'id' => $r['blog_id'] ], network_admin_url($link['url']));
