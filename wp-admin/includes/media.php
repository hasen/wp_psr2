...
function the_media_upload_tabs()
{
    ...
    if (!empty($tabs)) {
        ...
        foreach ($tabs as $callback => $text) {
            ...
            $href = add_query_arg(['tab' => $callback, 's' => false, 'paged' => false, 'post_mime_type' => false, 'm' => false]);
...
function wp_media_upload_handler()
{
    ...
    if (isset($_GET['tab']) && $_GET['tab'] == 'type_url') {
        $type = 'image';
        if (isset($_GET['type']) && in_array($_GET['type'], [ 'video', 'audio', 'file' ])) {
            $type = $_GET['type'];
        }
...
function image_align_input_fields($post, $checked = '')
{
    ...
    $alignments = ['none' => __('None'), 'left' => __('Left'), 'center' => __('Center'), 'right' => __('Right')];
...
function get_media_items($post_id, $errors)
{
    ...
    foreach ((array) $attachments as $id => $attachment) {
        ...
        if ($item = get_media_item($id, [ 'errors' => isset($errors[$id]) ? $errors[$id] : null])) {
...
function get_media_item($attachment_id, $args = null)
{
    ...
    if (($r['send'] || $thumbnail || $delete) && !isset($form_fields['buttons'])) {
        $form_fields['buttons'] = [ 'tr' => "\t\t<tr class='submit'><td></td><td class='savesend'>" . $r['send'] . " $thumbnail $delete</td></tr>\n" ];
    }
    $hidden_fields = [];
...
<div id="drag-drop-area">
	...
	<p class="drag-drop-info"><?php _e('Drop files here');
    ?></p>
	<p><?php _ex('or', 'Uploader: Drop files here - or - Select Files');
    ?></p>
	<p class="drag-drop-buttons"><input id="plupload-browse-button" type="button" value="<?php esc_attr_e('Select Files');
    ?>" class="button" /></p>
	</div>
</div>
<?php
...
<p class="max-upload-size"><?php printf(__('Maximum upload file size: %s.'), esc_html(size_format($max_upload_size)));
    ?></p>
<?php
...
<h3 class="media-title"><?php _e('Add media files from your computer');
    ?></h3>
...
<h3 class="media-title"><?php _e('Insert media from another website');
    ?></h3>
...
<?php 
	...
	resetImageData : function() {
		...
		else document.getElementById('status_img').innerHTML = '<img src="<?php echo esc_url(admin_url('images/no.png'));
    ?>" alt="" />';
	},
	...
	updateImageData : function() {
		...
		document.getElementById('status_img').innerHTML = '<img src="<?php echo esc_url(admin_url('images/yes.png'));
    ?>" alt="" />';
	},
	...
	getImageData : function() {
		...
		document.getElementById('status_img').innerHTML = '<img src="<?php echo esc_url(admin_url('images/spinner-2x.gif'));
...
<?php _e('All Tabs:');
    ?>
<a href="#" id="showall"><?php _e('Show');
    ?></a>
<a href="#" id="hideall" style="display:none;"><?php _e('Hide');
    ?></a>
</span>
<?php _e('Sort Order:');
    ?>
<a href="#" id="asc"><?php _e('Ascending');
    ?></a> |
<a href="#" id="desc"><?php _e('Descending');
    ?></a> |
<a href="#" id="clear"><?php _ex('Clear', 'verb');
    ?></a>
</div>
<form enctype="multipart/form-data" method="post" action="<?php echo esc_url($form_action_url);
    ?>" class="<?php echo $form_class;
...
<th><?php _e('Media');
    ?></th>
<th class="order-head"><?php _e('Order');
    ?></th>
...
<p class="ml-submit">
<?php submit_button(__('Save all changes'), 'button savebutton', 'save', false, [ 'id' => 'save-all', 'style' => 'display: none;' ]);
    ?>
<input type="hidden" name="post_id" id="post_id" value="<?php echo (int) $post_id;
    ?>" />
...
<div class="title"><?php _e('Gallery Settings');
    ?></div>
<table id="basic" class="describe"><tbody>
	<tr>
	<th scope="row" class="label">
		<label>
		<span class="alignleft"><?php _e('Link thumbnails to:');
    ?></span>
		</label>
	</th>
	<td class="field">
		<input type="radio" name="linkto" id="linkto-file" value="file" />
		<label for="linkto-file" class="radio"><?php _e('Image File');
    ?></label>

		<input type="radio" checked="checked" name="linkto" id="linkto-post" value="post" />
		<label for="linkto-post" class="radio"><?php _e('Attachment Page');
    ?></label>
	</td>
	</tr>
	...
	<th scope="row" class="label">
		<label>
		<span class="alignleft"><?php _e('Order images by:');
    ?></span>
		</label>
	</th>
	<td class="field">
		<select id="orderby" name="orderby">
			<option value="menu_order" selected="selected"><?php _e('Menu order');
    ?></option>
			<option value="title"><?php _e('Title');
    ?></option>
			<option value="post_date"><?php _e('Date/Time');
    ?></option>
			<option value="rand"><?php _e('Random');
    ?></option>
		</select>
	</td>
	</tr>
	...
	<th scope="row" class="label">
		<label>
		<span class="alignleft"><?php _e('Order:');
    ?></span>
		</label>
	</th>
	<td class="field">
		<input type="radio" checked="checked" name="order" id="order-asc" value="asc" />
		<label for="order-asc" class="radio"><?php _e('Ascending');
    ?></label>

		<input type="radio" name="order" id="order-desc" value="desc" />
		<label for="order-desc" class="radio"><?php _e('Descending');
    ?></label>
	</td>
	</tr>
	...
	<th scope="row" class="label">
		<label>
		<span class="alignleft"><?php _e('Gallery columns:');
    ?></span>
		</label>
	</th>
	<td class="field">
...
if (empty($_GET['post_mime_type']) && !empty($num_posts[$type])) {
    ...
    $type_links[] = '<li><a href="' . esc_url(add_query_arg(['post_mime_type' => 'all', 'paged' => false, 'm' => false])) . '"' . $class . '>' . __('All Types') . '</a>';
    foreach ($post_mime_types as $mime_type => $label) {
        ...
        $type_links[] = '<li><a href="' . esc_url(add_query_arg(['post_mime_type' => $mime_type, 'paged' => false])) . '"' . $class . '>' . sprintf(translate_nooped_plural($label[2], $num_posts[$mime_type]), '<span id="' . $mime_type . '-counter">' . number_format_i18n($num_posts[$mime_type]) . '</span>') . '</a>';
    }
/**
...
function edit_form_image_editor($post)
{
    ...
    ?>
		...
		<div class="imgedit-response" id="imgedit-response-<?php echo $attachment_id;
    ?>"></div>
    ...
    ?>"><img class="thumbnail" src="<?php echo set_url_scheme($thumb_url[0]);
    ?>" style="max-width:100%" alt="" /></p>
			<p><?php echo $image_edit_button;
    ?></p>
		</div>
		<div<?php if (! $open) {
    echo ' style="display:none"';
    ...
    echo wp_audio_shortcode([ 'src' => $att_url ]); elseif ($attachment_id && wp_attachment_is('video', $post)):
    ...
    echo wp_video_shortcode($attr); else :
	...
	<div class="wp_attachment_details edit-form-section">
		<p>
			<label for="attachment_caption"><strong><?php _e('Caption');
    ?></strong></label><br />
			<textarea class="widefat" name="excerpt" id="attachment_caption"><?php echo $post->post_excerpt;
    ?></textarea>
		</p>
	...
	<label for="attachment_content"><strong><?php _e('Description');
    ?></strong><?php
...
function attachment_submitbox_metadata()
{
	...
	<div class="misc-pub-section misc-pub-attachment">
		<label for="attachment_url"><?php _e('File URL:');
    ?></label>
    ...
    if (! empty($file_size)) : ?>
			<div class="misc-pub-section misc-pub-filesize">
				<?php _e('File size:');
    ?> <strong><?php echo size_format($file_size);
    ?></strong>
			</div>
			<?php
        endif;
    ...
    if (preg_match('#^(audio|video)/#', $post->post_mime_type)) {
         ...
         */
        $fields = apply_filters('media_submitbox_misc_sections', [
	...
	<?php
         ...
         * @param array $fields An array of the attachment metadata keys and labels.
         */
        $audio_fields = apply_filters('audio_submitbox_misc_sections', [
		...
		<div class="misc-pub-section misc-pub-audio misc-pub-<?php echo sanitize_html_class($key);
            ?>">
			<?php echo $label;
            ?> <strong><?php echo esc_html($meta['audio'][$key]);
            ?></strong>
		</div>
	<?php
