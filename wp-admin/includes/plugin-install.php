...
{
    ?>
	<p><?php printf(__('Plugins extend and expand the functionality of WordPress. You may automatically install plugins from the <a href="%1$s">WordPress Plugin Directory</a> or upload a plugin in .zip format by clicking the button at the top of this page.'), 'https://wordpress.org/plugins/');
    ?></p>
    ...
    if (is_wp_error($api_tags)) {
        ...
        echo wp_generate_tag_cloud($tags, [ 'single_text' => __('%s plugin'), 'multiple_text' => __('%s plugins') ]);
    }
...
function install_search_form($deprecated = true)
{
    ...
    ?><form class="search-form search-plugins" method="get">
		...
		<label class="screen-reader-text" for="typeselector"><?php _e('Search plugins by:');
    ?></label>
    ...
    ?></option>
		...
		<label><span class="screen-reader-text"><?php _e('Search Plugins');
    ?></span>
			<input type="search" name="s" value="<?php echo esc_attr($term) ?>" class="wp-filter-search" placeholder="<?php esc_attr_e('Search Plugins');
    ?>" />
		</label>
		<?php submit_button(__('Search Plugins'), 'button hide-if-js', false, false, [ 'id' => 'search-submit' ]);
    ?>
	</form><?php
...
function install_plugin_information()
{
    ...
    foreach ((array) $api->sections as $section_name => $content) {
        ...
        $href = add_query_arg(['tab' => $tab, 'section' => $section_name]);
        $href = esc_url($href);
...
<div id="<?php echo $_tab;
    ...
    if (! empty($api->active_installs)) {
        ?>
				<li><strong><?php _e('Active Installs:');
        ?></strong> <?php
    ...
    if (! empty($api->slug) && empty($api->external)) {
        ?>
				<li><a target="_blank" href="https://wordpress.org/plugins/<?php echo $api->slug;
    ...
    if (! empty($api->homepage)) {
        ?>
				<li><a target="_blank" href="<?php echo esc_url($api->homepage);
    ...
    if (! empty($api->donate_link) && empty($api->contributors)) {
        ?>
				<li><a target="_blank" href="<?php echo esc_url($api->donate_link);
    ...
    ?></h3>
			<?php wp_star_rating([ 'rating' => $api->rating, 'type' => 'percent', 'number' => $api->num_ratings ]);
    ?>
			<p aria-hidden="true" class="fyi-description"><?php printf(_n('(based on %s rating)', '(based on %s ratings)', $api->num_ratings), number_format_i18n($api->num_ratings));
    ?></p>
		<?php 
}
    ...
    if (! empty($api->ratings) && array_sum((array) $api->ratings) > 0) {
        ?>
			<h3><?php _e('Reviews');
        ?></h3>
			<p class="fyi-description"><?php _e('Read all reviews on WordPress.org or write your own!');
        ?></p>
			<?php
            foreach ($api->ratings as $key => $ratecount) {
				...
				<div class="counter-container">
						<span class="counter-label"><a href="https://wordpress.org/support/view/plugin-reviews/<?php echo $api->slug;
                ?>?filter=<?php echo $key;
                ?>"
						                               target="_blank" aria-label="<?php echo $aria_label;
                ...
                ?>px;"></span>
					...
					<span class="counter-count" aria-hidden="true"><?php echo number_format_i18n($ratecount);
                ?></span>
				</div>
				<?php
