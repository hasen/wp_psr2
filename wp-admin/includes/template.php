...
function wp_terms_checklist($post_id = 0, $args = [])
{
    ...
    if (is_array($r['popular_cats'])) {
        ...
        $args['popular_cats'] = get_terms($taxonomy, [
            'fields' => 'ids',
            'orderby' => 'count',
            'order' => 'DESC',
            'number' => 10,
    ...
    if ($descendants_and_self) {
        $categories = (array) get_terms($taxonomy, [
    ...
    if ($r['checked_ontop']) {
        ...
        $output .= call_user_func_array([ $walker, 'walk' ], [ $checked_categories, 0, $args ]);
    }
    // Then the rest of them
    $output .= call_user_func_array([ $walker, 'walk' ], [ $categories, 0, $args ]);
...
function wp_comment_reply($position = 1, $checkbox = false, $mode = 'single', $table_row = true)
{
     ...
     */
    $content = apply_filters('wp_comment_reply', '', [ 'position' => $position, 'checkbox' => $checkbox, 'mode' => $mode ]);
...
<form method="get">
<?php if ($table_row) : ?>
<table style="display:none;"><tbody id="com-reply"><tr id="replyrow" class="inline-edit-row" style="display:none;"><td colspan="<?php echo $wp_list_table->get_column_count();
    ?>" class="colspanchange">
<?php else : ?>
...
<?php endif;
	...
	<legend>
		<span class="hidden" id="editlegend"><?php _e('Edit Comment');
    ?></span>
		<span class="hidden" id="replyhead"><?php _e('Reply to Comment');
    ?></span>
		<span class="hidden" id="addhead"><?php _e('Add new Comment');
    ?></span>
	</legend>

	<div id="replycontainer">
	<label for="replycontent" class="screen-reader-text"><?php _e('Comment');
    ?></label>
	<?php
    $quicktags_settings = [ 'buttons' => 'strong,em,link,block,del,ins,img,ul,ol,li,code,close' ];
    wp_editor('', 'replycontent', [ 'media_buttons' => false, 'tinymce' => false, 'quicktags' => $quicktags_settings ]);
    ?>
	</div>
    ...
    ?></span>
	<span id="savebtn" style="display:none;"><?php _e('Update Comment');
    ?></span>
	<span id="replybtn" style="display:none;"><?php _e('Submit Reply');
    ?></span></a>
	<a href="#comments-form" class="cancel button-secondary alignleft"><?php _e('Cancel');
    ?></a>
...
<thead>
<tr>
<th class="left"><label for="<?php echo $meta_key_input_id;
    ?>"><?php _ex('Name', 'meta name') ?></label></th>
...
<option value="#NONE#"><?php _e('&mdash; Select &mdash;');
    ?></option>
<?php
...
<span id="enternew"><?php _e('Enter new');
    ?></span>
<span id="cancelnew" class="hidden"><?php _e('Cancel');
    ?></span></a>
<?php 
} else {
    ?>
...
<tr><td colspan="2">
<div class="submit">
<?php submit_button(__('Add Custom Field'), 'secondary', 'addmeta', false, [ 'id' => 'newmeta-submit', 'data-wp-lists' => 'add:the-list:newmeta' ]);
    ?>
</div>
...
function touch_time($edit = 1, $for_post = 1, $tab_index = 0, $multi = 0)
{
    ...
    if ($for_post) {
        $edit = ! (in_array($post->post_status, ['draft', 'pending']) && (!$post->post_date_gmt || '0000-00-00 00:00:00' == $post->post_date_gmt));
    }
...
<a href="#edit_timestamp" class="save-timestamp hide-if-no-js button"><?php _e('OK');
    ?></a>
<a href="#edit_timestamp" class="cancel-timestamp hide-if-no-js button-cancel"><?php _e('Cancel');
    ?></a>
</p>
<?php
...
function wp_import_upload_form($action)
{
    ...
    if (! empty($upload_dir['error'])) :
        ?><div class="error"><p><?php _e('Before you can upload your import file, you will need to fix the following error:');
    ?></p>
		<p><strong><?php echo $upload_dir['error'];
    ?></strong></p></div><?php
    else :
?>
...
function add_meta_box($id, $title, $callback, $screen = null, $context = 'advanced', $priority = 'default', $callback_args = null)
{
    ...
    $wp_meta_boxes[$page][$context][$priority][$id] = ['id' => $id, 'title' => $title, 'callback' => $callback, 'args' => $callback_args];
}
        ...
        foreach ([ 'high', 'core', 'default', 'low' ] as $priority) {
            if (isset($wp_meta_boxes[ $page ][ $context ][ $priority ])) {
                foreach ($wp_meta_boxes[ $page ][ $context ][ $priority ] as $box) {
                    ...
                    ?>
					<li class="control-section accordion-section <?php echo $hidden_class;
                    ?> <?php echo $open_class;
                    ?> <?php echo esc_attr($box['id']);
...
function add_settings_section($id, $title, $callback, $page)
{
    ...
    $wp_settings_sections[$page][$id] = ['id' => $id, 'title' => $title, 'callback' => $callback];
}
...
function add_settings_field($id, $title, $callback, $page, $section = 'default', $args = [])
{
    ...
    $wp_settings_fields[$page][$section][$id] = ['id' => $id, 'title' => $title, 'callback' => $callback, 'args' => $args];
}
...
function find_posts_div($found_action = '')
{
    ...
    ?>
			<button type="button" id="find-posts-close"><span class="screen-reader-text"><?php _e('Close media attachment panel');
    ?></button>
		</div>
    ...
    ?></label>
				...
				<input type="button" id="find-posts-search" value="<?php esc_attr_e('Search');
    ?>" class="button" />
				<div class="clear"></div>
			</div>
...
function convert_to_screen($hook_name)
{
    if (! class_exists('WP_Screen', false)) {
        ...
        return (object) [ 'id' => '_invalid', 'base' => '_are_belong_to_us' ];
    }
...
function _local_storage_notice()
{
    ...
    ?>
		<button type="button" class="button restore-backup"><?php _e('Restore the backup');
    ?></button>
	</p>
	<p class="help">
