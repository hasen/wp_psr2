...
function wp_ajax_ajax_tag_search()
{
    ...
    $results = get_terms($taxonomy, [ 'name__like' => $s, 'fields' => 'names', 'hide_empty' => false ]);
...
function wp_ajax_autocomplete_user()
{
    ...
    $include_blog_users = ($type == 'search' ? get_users([ 'blog_id' => $id, 'fields' => 'ID' ]) : []);
    $exclude_blog_users = ($type == 'add' ? get_users([ 'blog_id' => $id, 'fields' => 'ID' ]) : []);
...
function _wp_ajax_add_hierarchical_term()
{
    ...
    foreach ($names as $cat_name) {
        ...
        wp_terms_checklist(0, [ 'taxonomy' => $taxonomy->name, 'descendants_and_self' => $cat_id, 'selected_cats' => $checked_categories, 'popular_cats' => $popular_ids ]);
    ...
    if ($parent) { // Foncy - replace the parent and all its children
        ...
        wp_terms_checklist(0, ['taxonomy' => $taxonomy->name, 'descendants_and_self' => $term_id, 'selected_cats' => $checked_categories, 'popular_cats' => $popular_ids]);
...
function wp_ajax_dim_comment()
{
    ...
    if (in_array($current, [ 'unapproved', 'spam' ])) {
...
function wp_ajax_get_tagcloud()
{
    ...
    $tags = get_terms($taxonomy, [ 'number' => 45, 'orderby' => 'count', 'order' => 'DESC' ]);
...
function wp_ajax_replyto_comment($action)
{
    ...
    if (empty($post->post_status)) {
        wp_die(1);
    } elseif (in_array($post->post_status, ['draft', 'pending', 'trash'])) {
...
function wp_ajax_closed_postboxes()
{
    ...
    if (is_array($hidden)) {
        $hidden = array_diff($hidden, ['submitdiv', 'linksubmitdiv', 'manage-menu', 'create-menu']); // postboxes that are always shown
...
function wp_ajax_inline_save()
{
    ...
    if (! empty($data['post_name']) && in_array($post['post_status'], [ 'draft', 'pending' ])) {
...
function wp_ajax_save_attachment_order()
{
    ...
    foreach ($attachments as $attachment_id => $menu_order) {
        ...
        wp_update_post([ 'ID' => $attachment_id, 'menu_order' => $menu_order ]);
    }
...
function wp_ajax_send_attachment_to_editor()
{
    ...
    if (current_user_can('edit_post', $id)) {
        ...
        if (0 == $post->post_parent && $insert_into_post_id = intval($_POST['post_id'])) {
            wp_update_post([ 'ID' => $id, 'post_parent' => $insert_into_post_id ]);
        }
    }
...
function wp_ajax_get_revision_diffs()
{
    ...
    if (! $revisions = wp_get_post_revisions($post->ID, [ 'check_enabled' => false ])) {
        wp_send_json_error();
    }
...
function wp_ajax_parse_media_shortcode()
{
    ...
    if ('playlist' === $_REQUEST['type']) {
        ...
        wp_print_scripts([ 'froogaloop', 'wp-mediaelement' ]);
    }
...
function wp_ajax_search_plugins()
{
    ...
    $_SERVER['REQUEST_URI'] = add_query_arg(array_diff_key($_POST, [
        '_ajax_nonce' => null,
        'action'      => null,
