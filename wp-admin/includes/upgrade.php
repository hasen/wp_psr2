...
function wp_install_defaults($user_id)
{
    ...
    $wpdb->insert($wpdb->term_relationships, ['term_taxonomy_id' => $cat_tt_id, 'object_id' => 1]);
...
As a new WordPress user, you should go to <a href=\"%s\">your dashboard</a> to delete this page and create new pages for your content. Have fun!"), admin_url());
    ...
    $wpdb->insert($wpdb->postmeta, [ 'post_id' => 2, 'meta_key' => '_wp_page_template', 'meta_value' => 'default' ]);
    ...
    update_option('sidebars_widgets', [ 'wp_inactive_widgets' => [], 'sidebar-1' => [ 0 => 'search-2', 1 => 'recent-posts-2', 2 => 'recent-comments-2', 3 => 'archives-2', 4 => 'categories-2', 5 => 'meta-2',], 'array_version' => 3 ]);
    ...
    if ($allposts) :
        foreach ($allposts as $post) {
            ...
            if (!$cat && 0 != $post->post_category) { // If there's no result
                $wpdb->insert($wpdb->post2cat, ['post_id' => $post->ID, 'category_id' => $post->post_category]);
            }
        }
    endif;
}
...
function upgrade_160()
{
    ...
    if ($wp_current_db_version > 2541 && $wp_current_db_version <= 3091) {
        ...
        foreach ($objects as $object) {
            $wpdb->update($wpdb->posts, [    'post_status' => 'attachment',
...
function upgrade_230()
{
    ...
    foreach ($posts as $post) {
        ...
        $wpdb->insert($wpdb->term_relationships, ['object_id' => $post_id, 'term_taxonomy_id' => $tt_id]);
    }
    ...
    if ($wp_current_db_version < 3570) {
        ...
        foreach ($link_cats as $category) {
            ...
            $wpdb->insert($wpdb->term_taxonomy, ['term_id' => $term_id, 'taxonomy' => 'link_category', 'description' => '', 'parent' => 0, 'count' => 0]);
        ...
        if (!empty($links)) {
            foreach ($links as $link) {
                ...
                $wpdb->insert($wpdb->term_relationships, ['object_id' => $link->link_id, 'term_taxonomy_id' => $tt_id]);
            }
        }
        ...
        foreach ($links as $link) {
            ...
            $wpdb->insert($wpdb->term_relationships, ['object_id' => $link_id, 'term_taxonomy_id' => $tt_id]);
        }
    }
...
function make_site_theme_from_oldschool($theme_name, $template)
{
     ...
     */
    $files = ['index.php' => 'index.php', 'wp-layout.css' => 'style.css', 'wp-comments.php' => 'comments.php', 'wp-comments-popup.php' => 'comments-popup.php'];
