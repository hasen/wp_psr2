...
function populate_options()
{
    ...
    */
    ...
    $fat_options = [ 'moderation_keys', 'recently_edited', 'blacklist_keys', 'uninstall_plugins' ];
...
function populate_network($network_id = 1, $domain = '', $email = '', $site_name = '', $path = '/', $subdomain_install = false)
{
    ...
    if (1 == $network_id) {
        $wpdb->insert($wpdb->site, [ 'domain' => $domain, 'path' => $path ]);
        $network_id = $wpdb->insert_id;
    } else {
        $wpdb->insert($wpdb->site, [ 'domain' => $domain, 'path' => $path, 'id' => $network_id ]);
    }
...
--The Team @ SITE_NAME');
    ...
    if (! is_multisite()) {
        ...
        $wpdb->insert($wpdb->blogs, [ 'site_id' => $network_id, 'blog_id' => 1, 'domain' => $domain, 'path' => $path, 'registered' => current_time('mysql') ]);
        ...
        $page = wp_remote_get('http://' . $hostname, [ 'timeout' => 5, 'httpversion' => '1.1' ]);
        if (is_wp_error($page)) {
