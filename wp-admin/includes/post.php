...
function wp_edit_posts_query($q = false)
{
    ...
    if (isset($q['orderby'])) {
        $orderby = $q['orderby'];
    } elseif (isset($q['post_status']) && in_array($q['post_status'], [ 'pending', 'draft' ])) {
        $orderby = 'modified';
    }
...
function _admin_notice_post_locked()
{
    ...
    if ($locked) {
		...
		<div class="post-locked-avatar"><?php echo get_avatar($user->ID, 64);
        ?></div>
        ...
        ?>
		<p>
		<a class="button" href="<?php echo esc_url($sendback);
        ?>"><?php echo $sendback_text;
        ?></a>
		<?php if ($preview_link) {
    ?>
    ...
    ?></a>
        ...
        if ($override) {
            ?>
			<a class="button button-primary wp-tab-last" href="<?php echo esc_url(add_query_arg('get-post-lock', '1', wp_nonce_url(get_edit_post_link($post->ID, 'url'), 'lock-post_' . $post->ID)));
            ?>"><?php _e('Take over');
            ?></a>
			<?php
        ...
        ?>" width="16" height="16" alt="" /> <?php _e('Saving revision&hellip;');
        ?></span>
        ...
        ?>
			<p><a class="button button-primary wp-tab-last" href="<?php echo esc_url($sendback);
        ?>"><?php echo $sendback_text;
        ?></a></p>
		</div>
		<?php
