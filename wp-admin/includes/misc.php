...
function wp_doc_link_parse($content)
{
    ...
    for ($t = 0; $t < $count - 2; $t++) {
        ...
        if (T_STRING == $tokens[ $t ][0] && ('(' == $tokens[ $t + 1 ] || '(' == $tokens[ $t + 2 ])) {
            ...
            if ((isset($tokens[ $t - 2 ][1]) && in_array($tokens[ $t - 2 ][1], [ 'function', 'class' ])) || (isset($tokens[ $t - 2 ][0]) && T_OBJECT_OPERATOR == $tokens[ $t - 1 ][0])) {
...
function admin_color_scheme_picker($user_id)
{
    ...
    if (isset($_wp_admin_css_colors['fresh'])) {
        ...
        $_wp_admin_css_colors = array_filter(array_merge([ 'fresh' => '', 'light' => '' ], $_wp_admin_css_colors));
    }
	...
	<fieldset id="color-picker" class="scheme-list">
		<legend class="screen-reader-text"><span><?php _e('Admin Color Scheme');
    ?></span></legend>
		<?php
    ...
    ?>">
				<input name="admin_color" id="admin_color_<?php echo esc_attr($color);
    ?>" type="radio" value="<?php echo esc_attr($color);
    ...
    ?>" />
				<label for="admin_color_<?php echo esc_attr($color);
...
function wp_color_scheme_settings()
{
    ...
    if (! empty($_wp_admin_css_colors[ $color_scheme ]->icon_colors)) {
        ...
        $icon_colors = [ 'base' => '#82878c', 'focus' => '#00a0d2', 'current' => '#fff' ];
    }
...
function heartbeat_autosave($response, $data)
{
    if (! empty($data['wp_autosave'])) {
        ...
        if (is_wp_error($saved)) {
            $response['wp_autosave'] = [ 'success' => false, 'message' => $saved->get_error_message() ];
        } elseif (empty($saved)) {
            $response['wp_autosave'] = [ 'success' => false, 'message' => __('Error while saving.') ];
        } else {
            /* translators: draft saved date format, see https://secure.php.net/date */
            $draft_saved_date_format = __('g:i:s a');
            /* translators: %s: date and time */
            $response['wp_autosave'] = [ 'success' => true, 'message' => sprintf(__('Draft saved at %s.'), date_i18n($draft_saved_date_format)) ];
        }
    }
