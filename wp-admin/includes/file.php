...
function _wp_handle_upload(&$file, $overrides, $time, $action)
{
    ...
    if ($test_type) {
        ...
        if ((! $type || !$ext) && ! current_user_can('unfiltered_upload')) {
            return call_user_func_array($upload_error_handler, [ &$file, __('Sorry, this file type is not permitted for security reasons.') ]);
        }
        if (! $type) {
            $type = $file['type'];
        }
...
<div class="ftp-username">
	<label for="username">
		<span class="field-title"><?php echo $label_user;
    ?></span>
...
<div class="ftp-password">
	<label for="password">
		<span class="field-title"><?php echo $label_pass;
    ?></span>
...
<fieldset id="ssh-keys"<?php echo $hidden_class;
    ?>">
<legend><?php _e('Authentication Keys');
    ?></legend>
...
<?php
	...
	<p class="request-filesystem-credentials-action-buttons">
		<button class="button cancel-button" data-js-action="close" type="button"><?php _e('Cancel');
    ?></button>
