...
if (! is_multisite()) {
    ...
    $submenu[ 'index.php' ][10] = [ sprintf(__('Updates %s'), "<span class='update-plugins count-{$update_data['counts']['total']}' title='{$update_data['title']}'><span class='update-count'>" . number_format_i18n($update_data['counts']['total']) . "</span></span>"), $cap, 'update-core.php'];
    unset($cap);
}
...
$menu[10] = [ __('Media'), 'upload_files', 'upload.php', '', 'menu-top menu-icon-media', 'menu-media', 'dashicons-admin-media' ];
    ...
    $submenu['upload.php'][10] = [ _x('Add New', 'file'), 'upload_files', 'media-new.php'];
    $i = 15;
...
$menu[15] = [ __('Links'), 'manage_links', 'link-manager.php', '', 'menu-top menu-icon-links', 'menu-links', 'dashicons-admin-links' ];
    ...
    $submenu['link-manager.php'][10] = [ _x('Add New', 'link'), 'manage_links', 'link-add.php' ];
...
$types = (array) get_post_types(['show_ui' => true, '_builtin' => false, 'show_in_menu' => true ]);
$builtin = [ 'post', 'page' ];
...
$menu[60] = [ __('Appearance'), $appearance_cap, 'themes.php', '', 'menu-top menu-icon-appearance', 'menu-appearance', 'dashicons-admin-appearance' ];
    $submenu['themes.php'][5] = [ __('Themes'), $appearance_cap, 'themes.php' ];
    ...
    if (current_theme_supports('custom-header') && current_user_can('customize')) {
        ...
        $submenu['themes.php'][15] = [ __('Header'), $appearance_cap, esc_url($customize_header_url), '', 'hide-if-no-customize' ];
    }

    if (current_theme_supports('custom-background') && current_user_can('customize')) {
        $customize_background_url = add_query_arg([ 'autofocus' => [ 'control' => 'background_image' ] ], $customize_url);
        $submenu['themes.php'][20] = [ __('Background'), $appearance_cap, esc_url($customize_background_url), '', 'hide-if-no-customize' ];
    }
...
if (current_user_can('list_users')) {
    ...
    if (current_user_can('create_users')) {
        ...
        $submenu['users.php'][10] = [_x('Add New', 'user'), 'promote_users', 'user-new.php'];
    }

    $submenu['users.php'][15] = [__('Your Profile'), 'read', 'profile.php'];
} else {
    $_wp_real_parent_file['users.php'] = 'profile.php';
    $submenu['profile.php'][5] = [__('Your Profile'), 'read', 'profile.php'];
    if (current_user_can('create_users')) {
...
$menu[75] = [ __('Tools'), 'edit_posts', 'tools.php', '', 'menu-top menu-icon-tools', 'menu-tools', 'dashicons-admin-tools' ];
    ...
    if (is_multisite() && !is_main_site()) {
        $submenu['tools.php'][25] = [ __('Delete Site'), 'delete_site', 'ms-delete-site.php' ];
    }
    if (! is_multisite() && defined('WP_ALLOW_MULTISITE') && WP_ALLOW_MULTISITE) {
        $submenu['tools.php'][50] = [__('Network Setup'), 'manage_options', 'network.php'];
    }
