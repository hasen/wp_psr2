...
class Akismet_Admin
{
    ...
    {
        if (class_exists('Jetpack')) {
            $hook = add_submenu_page('jetpack', __('Akismet', 'akismet'), __('Akismet', 'akismet'), 'manage_options', 'akismet-key-config', [ 'Akismet_Admin', 'display_page' ]);
        } else {
            $hook = add_options_page(__('Akismet', 'akismet'), __('Akismet', 'akismet'), 'manage_options', 'akismet-key-config', [ 'Akismet_Admin', 'display_page' ]);
        }
    ...
    public static function load_resources()
    {
        ...
        if (in_array($hook_suffix, [
            ...
            wp_register_style('akismet.css', plugin_dir_url(__FILE__) . '_inc/akismet.css', [], AKISMET_VERSION);
            wp_enqueue_style('akismet.css');

            wp_register_script('akismet.js', plugin_dir_url(__FILE__) . '_inc/akismet.js', ['jquery', 'postbox'], AKISMET_VERSION);
    ...
    public static function check_for_spam_button($comment_status)
    {
        ...
        if (function_exists('plugins_url')) {
            $link = add_query_arg([ 'action' => 'akismet_recheck_queue' ], admin_url('admin.php'));
        } else {
            $link = add_query_arg([ 'page' => 'akismet-admin', 'recheckqueue' => 'true', 'noheader' => 'true' ], admin_url('edit-comments.php'));
        }
    ...
    public static function recheck_queue()
    {
        ...
        if (isset($_POST['limit']) && isset($_POST['offset'])) {
            $paginate = $wpdb->prepare(" LIMIT %d OFFSET %d", [ $_POST['limit'], $_POST['offset'] ]);
        }
        ...
        foreach ((array) $moderation as $c) {
            ...
            if ('true' == $response[1]) {
                ...
                Akismet::update_comment_history(
                    ...
                    '',
                    'recheck-error',
                    [ 'response' => substr($response[1], 0, 50) ]
                );
                ++$result_counts['error'];
            }
    ...
    public static function get_akismet_user($api_key)
    {
        ...
        $subscription_verification = Akismet::http_post(Akismet::build_query([ 'key' => $api_key, 'blog' => get_bloginfo('url') ]), 'get-subscription');
    ...
    public static function get_stats($api_key)
    {
        ...
        foreach ([ '6-months', 'all' ] as $interval) {
            $response = Akismet::http_post(Akismet::build_query([ 'blog' => get_bloginfo('url'), 'key' => $api_key, 'from' => $interval ]), 'get-stats');
    ...
    {
        Akismet::view('notice', [ 'type' => 'version' ]);
    }
    ...
    public static function display_configuration_page()
    {
        ...
        if (empty(self::$notices)) {
            ...
            if (! empty($stat_totals['all']) && isset($stat_totals['all']->time_saved) && $akismet_user->status == 'active' && $akismet_user->account_type == 'free-api-key') {
                ...
                Akismet::view('notice', [ 'type' => 'active-notice', 'time_saved' => $time_saved ]);
            }
            
            if (!empty($akismet_user->limit_reached) && in_array($akismet_user->limit_reached, [ 'yellow', 'red' ])) {
                Akismet::view('notice', [ 'type' => 'limit-reached', 'level' => $akismet_user->limit_reached ]);
            }
        }
        
        if (!isset(self::$notices['status']) && in_array($akismet_user->status, [ 'cancelled', 'suspended', 'missing', 'no-sub' ])) {
            Akismet::view('notice', [ 'type' => $akismet_user->status ]);
        }
    ...
    public static function display_notice()
    {
        ...
        if (in_array($hook_suffix, [ 'jetpack_page_akismet-key-config', 'settings_page_akismet-key-config', 'edit-comments.php' ]) && (int) get_option('akismet_alert_code') > 0) {
        ...
        } elseif (in_array($hook_suffix, [ 'jetpack_page_akismet-key-config', 'settings_page_akismet-key-config' ]) && Akismet::get_api_key()) {
            self::display_status();
        }
    }
