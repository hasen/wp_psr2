...
class multibyte_patch
{
    ...
    public function incoming_trackback($commentdata)
    {
        ...
        $from_encoding = str_replace([ ',', ' ' ], '', strtoupper(trim($from_encoding)));
    ...
    public function wplink_js(&$scripts)
    {
        ...
        if (! is_admin() || ! isset($pagenow) || ! in_array($pagenow, [ 'post.php', 'post-new.php' ]) || ! is_file($file)) {
            return;
        }

        $debug_qs = defined('SCRIPT_DEBUG') && SCRIPT_DEBUG ? '?sd=1' : '';

        $scripts->add('wplink', plugin_dir_url(__FILE__) . "wplink.php{$debug_qs}", [ 'jquery', 'wp-a11y' ], false, 1);
    }
    ...
    {
        $is_query_funcs = [ 'is_feed', 'is_404', 'is_search', 'is_tax', 'is_front_page', 'is_home', 'is_attachment', 'is_single', 'is_page', 'is_category', 'is_tag', 'is_author', 'is_date', 'is_archive', 'is_paged' ];
    ...
    public function filters_after_setup_theme()
    {
        ...
        if (false !== $this->conf['patch_force_character_count']) {
            if ('characters_including_spaces' != _x('words', 'Word count type. Do not translate!')) {
                add_filter('gettext_with_context', [ $this, 'force_character_count' ], 10, 3);
            }
        }

        if (false !== $this->conf['patch_force_twentytwelve_open_sans_off'] && 'twentytwelve' == get_template()) {
            add_action('wp_enqueue_scripts', [ $this, 'force_twentytwelve_open_sans_off' ], 99);
            add_action('admin_print_styles-appearance_page_custom-header', [ $this, 'force_twentytwelve_open_sans_off' ], 99);
        }

        if (false !== $this->conf['patch_force_twentythirteen_google_fonts_off'] && 'twentythirteen' == get_template()) {
            add_action('wp_enqueue_scripts', [ $this, 'force_twentythirteen_google_fonts_off' ], 99);
            add_action('admin_print_styles-appearance_page_custom-header', [ $this, 'force_twentythirteen_google_fonts_off' ], 99);
        }

        if (false !== $this->conf['patch_force_twentyfourteen_google_fonts_off'] && 'twentyfourteen' == get_template()) {
            add_action('wp_enqueue_scripts', [ $this, 'force_twentyfourteen_google_fonts_off' ], 99);
            add_action('admin_print_scripts-appearance_page_custom-header', [ $this, 'force_twentyfourteen_google_fonts_off' ], 99);
        }

        if (false !== $this->conf['patch_force_twentyfifteen_google_fonts_off'] && 'twentyfifteen' == get_template()) {
            add_action('wp_enqueue_scripts', [ $this, 'force_twentyfifteen_google_fonts_off' ], 99);
        ...
        if (false !== $this->conf['patch_force_twentysixteen_google_fonts_off'] && 'twentysixteen' == get_template()) {
            add_action('wp_enqueue_scripts', [ $this, 'force_twentysixteen_google_fonts_off' ], 99);
    ...
    {
        // add filter
        add_filter('preprocess_comment', [ $this, 'preprocess_comment' ], 99);

        if (false !== $this->conf['patch_incoming_pingback']) {
            add_filter('pre_remote_source', [ $this, 'pre_remote_source' ], 10, 2);
        }

        if (false !== $this->conf['patch_wp_trim_excerpt']) {
            add_filter('excerpt_length', [ $this, 'excerpt_mblength' ], 99);
            add_filter('excerpt_more', [ $this, 'excerpt_more' ], 9);
        }

        if (false !== $this->conf['patch_get_comment_excerpt']) {
            add_filter('get_comment_excerpt', [ $this, 'get_comment_excerpt' ], 10, 3);
        }
        ...
        if (false !== $this->conf['patch_bp_create_excerpt']) {
            add_filter('bp_create_excerpt', [ $this, 'bp_create_excerpt' ], 99);
            add_filter('bp_get_activity_content_body', [ $this, 'bp_get_activity_content_body' ], 99);
        }

        if (method_exists($this, 'wp_trim_words') && false !== $this->conf['patch_wp_trim_words']) {
            add_filter('wp_trim_words', [ $this, 'wp_trim_words' ], 99, 4);
        }

        // add action
        add_action('wp', [ $this, 'query_based_settings' ]);

        if (method_exists($this, 'process_search_terms') && false !== $this->conf['patch_process_search_terms']) {
            add_action('sanitize_comment_cookies', [ $this, 'process_search_terms' ]);
        }

        if (method_exists($this, 'wp_mail') && false !== $this->conf['patch_wp_mail']) {
            add_action('phpmailer_init', [ $this, 'wp_mail' ]);
        }

        if (method_exists($this, 'admin_custom_css') && false !== $this->conf['patch_admin_custom_css']) {
            add_action('admin_enqueue_scripts', [ $this, 'admin_custom_css' ], 99);
            add_action('customize_controls_enqueue_scripts', [ $this, 'admin_custom_css' ], 99);
        }

        if (false !== $this->conf['patch_wplink_js']) {
            add_action('wp_default_scripts', [ $this, 'wplink_js' ], 9);
        }

        add_action('after_setup_theme', [ $this, 'filters_after_setup_theme' ], 99);
    }
